...
function Invest() {
  ...
  const [systemStatus, setSystemStatus] = useState({Interest:0, LTV: 0, eduPrice: 0, deposit: 0, Issued: 0})
  ...
  async function protocolStatus() {
    ...
    const providerUrl = "https://neoxt4seed1.ngd.network/";
    const vat = Address.VatContractAddress;
    const ilk = ethers.encodeBytes32String('WGAS10'); // 'WXRP'를 bytes32로 인코딩

    // ERC-20 토큰 및 타겟 주소
    const tokenAddress = "0x1CE16390FD09040486221e912B87551E4e44Ab17"; // ERC-20 토큰 주소
    const targetAddress = "0xACa8F1491a9EA47e3401E066F16723B85AC79357"; // 조회할 지갑 주소
    // ERC-20 표준 ABI (balanceOf와 decimals 함수만 포함)
    const erc20Abi = [
                ...
                <div style={{paddingTop:"20px"}}/>
                      ...
                      <div className="flex">
                        <div className="relative">
                          <div className="relative mr-1.5 rounded-full bg-white">
                              <img class="w-10 h-10 rounded-full" src={"https://carrot-fi.xyz/images/ic_gas_token.svg"} alt=""/>
                          
                          </div>
                        </div>
                      </div>
                      <p className="mx-4 text-xl font-bold text-neutral-800">Collateral : GAS</p>
                    </div>
                  ...
                  <div className="flex flex-col">
                    ...
                    <div className="grid grid-cols-[1fr_1fr] gap-3">
                        ...
                        <p className="text-neutral-600 text-right">{userPosition.ink} GAS</p>
                      </div>
                      <div className="grid grid-cols-[1fr_1fr] gap-3">
                        <p className="font-semibold text-left">Debt</p>
                        <p className="text-neutral-600 text-right">{userPosition.art} NSD</p>
                      </div>
                    </div>
                  </div>
          ...
          <OverBox>
              <SubTemplateBlockVertical>                
                  ...
                  <div className="flex flex-col">
                    <div className="flex flex-col gap-2 w-full pt-5">
                      <div className="grid grid-cols-[1fr_1fr] gap-3">
                        <p className="text-left">GAS (Supra Oracle)</p>
                        <p className="text-neutral-600 text-right">${systemStatus.eduPrice}</p>
                      </div>
                      <div className="grid grid-cols-[1fr_1fr] gap-3">
                        <p className="text-left">Deposited</p>
                        <p className="text-neutral-600 text-right">{systemStatus.deposit} GAS</p>
                      </div>
                      <div className="grid grid-cols-[1fr_1fr] gap-3">
                        <p className="text-left">Issued</p>
                        <p className="text-neutral-600 text-right">{systemStatus.Issued} NSD</p>
                      </div>
                    </div>
                  </div>
                </div>
